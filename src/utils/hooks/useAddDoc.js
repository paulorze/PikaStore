...
const useAddDoc = ({coleccion, objeto, navigate = false, carritoVaciar = null, refreshItems = null, setRefreshItems = null})=> {
    let endpointCollection = collection (db, coleccion);
    addDoc(endpointCollection, objeto).then(res => {navigate 
        ? Swal.fire({
            icon: 'success',
        ...
        }).then((result) => {if (result.isConfirmed || result.isDismissed){navigate('/'); carritoVaciar()}})
        : Swal.fire({
            icon: 'success',
